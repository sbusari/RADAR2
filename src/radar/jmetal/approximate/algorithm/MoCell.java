...
import radar.jmetal.util.CrowdingArchive;
import radar.jmetal.util.Distance;
import radar.jmetal.util.JMException;
import radar.jmetal.util.Neighborhood;
...
public class MoCell extends Algorithm{
  ...
  public MoCell(Problem problem, Parser parserEngine) {
	     super (problem) ;
  ...
  public SolutionDetail execute() throws JMException, ClassNotFoundException {
    ...
    NonDominatedSolutionList nonDomSolution = new NonDominatedSolutionList();
    ...
    for (int i = 0; i < populationSize; i++){
      ...
      dominatedSolution.add(individual);
      nonDomSolution.addExactSolution(individual);
      evaluations++;
    }
    
    if (evaluations == maxEvaluations){
    	for (int i =0; i < nonDomSolution.size(); i ++){
    		archive.add(nonDomSolution.get(i));
    	}
    }
