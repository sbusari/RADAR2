...
public abstract class Settings {
	...
	protected DecisionVectorType typeOfDecisionVector_;
	 ...
	 * Constructor
	 */
	public Settings (String problemName, DecisionVectorType typeOfDecisionVector) {
   ...
   public Algorithm configure(Properties configuration) throws JMException {return null;} ;
	...
	public final Algorithm configure(HashMap settings) throws JMException, IllegalArgumentException, IllegalAccessException, ClassNotFoundException {
		if (settings != null) {
					...
					if (fields[i].getType().equals(int.class) ||
						...
						if (settings.containsKey(fields[i].getName())) {
							if (fields[i].getName().equals("mutationProbability_") &&
									value == null) {
								if (problem_.getSolutionType().getClass() != ArrayBitVectorSolutionType.class) {
									...
									value = 1.0 / length;
									//System.out.println("VALUE IS NULL AND VALUE CALCULATED: "+ value);
								}
								fields[i].setDouble(this, value);
							} // if
							else {
								System.out.println("ENTER THE ELSE JARE");
								fields[i].setDouble(this, value);
							}
						}
					} else {
						...
						if (value!=null) {
							if (fields[i].getType().equals(Crossover.class)) {
								Object value2 = CrossoverFactory.getCrossoverOperator((String)value,settings,typeOfDecisionVector_);
								value = value2;
							}

							if (fields[i].getType().equals(Mutation.class)) {
								Object value2 = MutationFactory.getMutationOperator((String)value, settings,typeOfDecisionVector_);
								value = value2;
							}
			...
			//At this point, we get the path where the true pareto fronts will be stored
