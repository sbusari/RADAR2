...
package radar.jmetal.experiments;
...
import radar.jmetal.experiments.util.RBoxplot;
import radar.jmetal.experiments.util.RWilcoxon;
import radar.jmetal.experiments.util.RunExperiment;
import radar.jmetal.experiments.util.Statistics;
import radar.jmetal.qualityIndicator.Coverage;
import radar.jmetal.qualityIndicator.Epsilon;
import radar.jmetal.qualityIndicator.GenerationalDistance;
import radar.jmetal.qualityIndicator.Hypervolume;
import radar.jmetal.qualityIndicator.InvertedGenerationalDistance;
import radar.jmetal.qualityIndicator.MetricsUtil;
import radar.jmetal.qualityIndicator.Spread;
...
public abstract class Experiment {
  ...
  public  OptimisationType optimisationType_;
  public  boolean useExactAsTrueFront_;
  ...
  public String IndicatorDirectory_;
  ...
  // in the constructor
  public  LinkedHashMap<String, SolutionDetail> solutionDetails_;
  ...
  public Experiment() {
    ...
    optimisationType_ = null;
    problemsSettings_ = null;
    useExactAsTrueFront_ =false;
    ...
    IndicatorDirectory_ = "Indicators";
    ...
    indicatorMinimize_.put("Coverage", true);
    solutionDetails_ = new LinkedHashMap<String, SolutionDetail>();
   ...
   * Runs the experiment
   */
  public LinkedHashMap<String, SolutionDetail> runExperiment(int numberOfThreads) throws JMException, IOException,ModelException, RuntimeException, InterruptedException {
    ...
    return this.solutionDetails_;
    
    // Inicio modificación ReferenceFronts
    //generateQualityIndicators();
    // Fin modificación ReferenceFronts
  }
  public synchronized void setAllRunSolutionDetails_(String algorithm, SolutionDetail solutionDetail){
	  this.solutionDetails_.put(algorithm, solutionDetail);
  }
  /**
   * Runs the experiment
   */
  public void runExperiment() throws JMException, IOException,ModelException, RuntimeException, InterruptedException {
    runExperiment(1);
  } // runExperiment
  ...
  public void initExperiment() {
    ...
    map_.put("optimisationType", optimisationType_);
    map_.put("useExactAsTrueFront", useExactAsTrueFront_);
  ...
  public void runCompleteExperiment() throws JMException, IOException,ModelException, RuntimeException, InterruptedException {
    runCompleteExperiment(1);
  }

  public void runCompleteExperiment(int numberOfThreads) throws JMException, IOException,ModelException, RuntimeException, InterruptedException {
    this.initExperiment();
  ...
  public static void main(String[] args) throws JMException, IOException ,ModelException, RuntimeException, InterruptedException{
  }
  ...
  public void WriteIndicatorResultsToCSV ()throws FileNotFoundException, IOException{
	  
	   System.out.println("=======================WRTING INDICATORS TO CSV============================" );
	    latexDirectory_ = experimentBaseDirectory_ + "/" + latexDirectory_;
	    IndicatorDirectory_ = experimentBaseDirectory_ + "/" + IndicatorDirectory_ + "/" + experimentName_ ;
    	Vector[][][] data = new Vector[indicatorList_.length][][];
 	    for (int indicator = 0; indicator < indicatorList_.length; indicator++) {
 	      // A data vector per problem [i][indicator]
	    ...
	    File statOutput;
	    statOutput = new File(IndicatorDirectory_);	    
	    if (!statOutput.exists()) {
	      boolean result = new File(IndicatorDirectory_).mkdirs();
	    }
	    String statFile = IndicatorDirectory_ ;	    		
	    printIndicatorsToCSV (statFile + "/"+ "Indicator"+"."+"csv", data);	   
	    
	  
  }
  void printIndicatorsToCSV(String fileName, Vector[][][] data)throws IOException{
	   
	  //System.out.println("File name: "+fileName );
	  FileWriter os = new FileWriter(fileName, true); 
	  
	  String header = "Problem"+ ","+ "Partition";		
	  String subHeader = ""+ ","+ "";
	  List<String> Header = null;
	  List<String> Header2 = null;
	  Map<String, String> problemHeader = new HashMap<String, String>();
	  Header  = new ArrayList<String>();
	  Header2  = new ArrayList<String>();
	  for (int j = 0; j < (indicatorList_.length); j++){	
		 for(int i =0; i< algorithmNameList_.length; i++){
			 Header2.add(algorithmNameList_[i]);
			 Header.add(indicatorList_[j]);
		 }
	  }
	  ...
	  for (int i =0; i< Header2.size(); i++ ){
		  subHeader += ", "+ Header2.get(i);
	  }
	  header = header +  "\n";
	  subHeader = subHeader + "\n";
	  for (int k = 0; k < problemList_.length; k++) {
		  
		  problemHeader.put(problemList_[k], subHeader);
	  }
	  ...
	  for (int i = 0; i < problemList_.length; i++) {
		  os.write(header);
		  os.write(subHeader);
		  String part_rec = problemList_[i] + "," + "P";
		  for (int indicator = 0; indicator < indicatorList_.length; indicator++){
			  String record ="";
			  for (int j = 0; j < (algorithmNameList_.length); j++) {
				  Vector v = data[indicator][i][j];	//NO OF RUNS				  
				  calculateStatistics(v, statValues);
				  record += "," + String.valueOf(new DecimalFormat("#.############").format((Double) statValues.get("mean"))); 					  
			  }			  
			  part_rec = part_rec + record;
		  } 
		  part_rec = part_rec +"\n";
		  os.write(part_rec);
   ...
   */
  public void generateQualityIndicators(boolean useExact) {
    ...
    if (indicatorList_.length > 0) {
      ...
      for (int algorithmIndex = 0; algorithmIndex < algorithmNameList_.length; algorithmIndex++) {
        ...
        for (int problemIndex = 0; problemIndex < problemList_.length; problemIndex++) {
          ...
          // note that this has to be changed if more than one problem are implemented (using the problemIndex)
          String paretoFrontPath = frontPath_[problemIndex];
          String paretoFrontDecisionVectorPath =  experimentBaseDirectory_ +  "/referenceFronts" + "/"+ problemList_[problemIndex] + ".VarRef";

          /*if (useExact){
        	  paretoFrontPath = experimentBaseDirectory_ +  "/referenceFronts" + "/"+ problemList_[problemIndex] + ".rf";
              paretoFrontDecisionVectorPath =  experimentBaseDirectory_ +  "/referenceFronts" + "/"+ problemList_[problemIndex] + ".VarRef";
          }else{
        	  paretoFrontPath = frontPath_[problemIndex];
              paretoFrontDecisionVectorPath =  experimentBaseDirectory_ +  "/referenceFronts" + "/"+ problemList_[problemIndex] + ".VarRef";
          }*/
          ...
          for (String anIndicatorList_ : indicatorList_) {
            ...
            for (int numRun = 0; numRun < independentRuns_; numRun++) {
              ...
              String solutionFrontFileDecisionSpace = problemDirectory + "/VAR." + numRun;
              String qualityIndicatorFile = problemDirectory;
              double value = 0;

              double[][] trueFront =  new Hypervolume().utils_.readFront(paretoFrontPath);
              List<String> trueFrontDecisionSpace =  Helper.readDecisionVectorsString(paretoFrontDecisionVectorPath);	
             ...
             /* if (anIndicatorList_.equals("Coverage")) {
            	  Coverage indicators = new Coverage();
            	  List<String> solutionFrontDecisionSpace = new Helper().readDecisionVectorsString(solutionFrontFileDecisionSpace);
	              value = indicators.coverage(solutionFrontDecisionSpace, trueFrontDecisionSpace, trueFront[0].length);
              ...
              if (anIndicatorList_.equals("Coverage")) {
	              ...
	              if(solutionFront != null  && solutionFront.length > 0){
	                value = indicators.coverage(solutionFront, trueFront, trueFront[0].length);
	              }
	              qualityIndicatorFile = qualityIndicatorFile + "/Coverage" ;
	          }
        ...
        for (int problemIndex = 0; problemIndex < problemList_.length; problemIndex++) {
          ...
          for (String anIndicatorList_ : indicatorList_) {
            ...
            for (int numRun = 0; numRun < independentRuns_; numRun++) {
              ...
              double[][] trueFront =  new Hypervolume().utils_.readFront(paretoFrontPath);
              List<String> trueFrontDecisionSpace =  new Helper().readDecisionVectorsString(paretoFrontDecisionVectorPath);	
  ...
  public void generateReferenceFronts(int problemIndex) {
    ...
    String decisionVariablePath = referenceFrontDirectory + "/" + problemList_[problemIndex] + ".VarRef";
    MetricsUtil metricsUtils = new MetricsUtil();
    NonDominatedSolutionList solutionSet = new NonDominatedSolutionList();
    NonDominatedSolutionList solutionSetDecisionVector = new NonDominatedSolutionList();
    String outputParetoFrontFilePathDecisionVector ="";
    for (String anAlgorithmNameList_ : algorithmNameList_) {
    	String theExactAlg = ConfigSetting.DEFAULT_EXACT_ALGORITHM;
    	 //if (optimisationType_ == OptimisationType.EXACT || (optimisationType_ == OptimisationType.BOTH && anAlgorithmNameList_.contains(theExactAlg)) ){
    		if (optimisationType_ == OptimisationType.EXACT && useExactAsTrueFront_ == true){
    		 String problemDirectory = experimentBaseDirectory_ + "/data/" + anAlgorithmNameList_ +
                     "/" + problemList_[problemIndex];
           	  //for (int numRun = 0; numRun < independentRuns_; numRun++) {
           	        String outputParetoFrontFilePath;
           	        outputParetoFrontFilePath = problemDirectory + "/FUN." + 0;
           	        outputParetoFrontFilePathDecisionVector = problemDirectory + "/VAR." + 0;
           	        String solutionFrontFile = outputParetoFrontFilePath;
           	        metricsUtils.readNonDominatedSolutionSet(solutionFrontFile, solutionSet,optimisationType_);
           	        if (solutionSet.size() > 0){
        	        	metricsUtils.readNonDominatedSolutionSetString(outputParetoFrontFilePathDecisionVector,solutionSet.get(0).getNumberOfObjectives(), solutionSetDecisionVector);
        	        }    
           	  //} // for
    	 }
        ...
        // if the reference file for exact does not exist, use approximate referecne front by setting the useExactAsTrueFront_ to false.
        if (useExactAsTrueFront_ == true){
        	File exactDirectory;
            String exactReferenceFrontDirectory = referenceFrontDirectory + "/" + problemList_[problemIndex] + ".rf";
            exactDirectory = new File(exactReferenceFrontDirectory);
        ...
        if (solutionSet.size() <= 0 && useExactAsTrueFront_ == false ){ // read all the alg
        	for (String anAlgorithmNameList_ : algorithmNameList_) {
        		 String problemDirectory = experimentBaseDirectory_ + "/data/" + anAlgorithmNameList_ +
                         "/" + problemList_[problemIndex];
               	  for (int numRun = 0; numRun < independentRuns_; numRun++) {
               	        String outputParetoFrontFilePath;
               	        outputParetoFrontFilePath = problemDirectory + "/FUN." + numRun;
               	        outputParetoFrontFilePathDecisionVector = problemDirectory + "/VAR." + numRun;
               	        String solutionFrontFile = outputParetoFrontFilePath;
               	        metricsUtils.readNonDominatedSolutionSet(solutionFrontFile, solutionSet, optimisationType_);
               	        if (solutionSet.size() > 0){
               	        	metricsUtils.readNonDominatedSolutionSetString(outputParetoFrontFilePathDecisionVector,solutionSet.get(0).getNumberOfObjectives(), solutionSetDecisionVector);
    ...
    solutionSet.printOptimalObjectivesToFile(frontPath_[problemIndex]);
    solutionSetDecisionVector.printDecisionVariables(decisionVariablePath);
  } // generateReferenceFronts
  ...
  public void generateLatexTables() throws FileNotFoundException, IOException,ModelException, RuntimeException, InterruptedException {
    latexDirectory_ = experimentBaseDirectory_ + "/" + latexDirectory_;
  ...
  public void generateRBoxplotScripts(int rows,
                                      ...
                                      String prefix,
                                      boolean notch,
                                      Experiment experiment) throws FileNotFoundException, IOException,ModelException, RuntimeException, InterruptedException {
